...
public class UserMeetingRegistrationFormService : IUserMeetingRegistrationFormService
{
    ...
    public UserMeetingRegistrationFormService(IUnitOfWork uow, IPasswordGeneratorService passwordService, UserManager<ApplicationUser> userManager, ISerilogLogger logger,IEmailService emailService)
    {
        _uow = uow;
    ...
    public async Task<Result<IEnumerable<UserMeetingRegistrationFormDto>>> GetAllAsync()
    {
        try
        {
            var allForms = await _uow.CreateRepository<IUserMeetingRegistrationFormRepository>().GetAllAsync();

            if (!allForms.Any())
            {
                _logger.LogInformation("No UserRegistrationForms found");
    ...
    public async Task<Result> AddAsync(UserMeetingRegistrationSparseDto? userInput)
    {
        try
        {
            ...
            var userIsAlreadyRegistered = await _uow.CreateRepository<IUserMeetingRegistrationFormRepository>()
            ...
            await _uow.CreateRepository<IUserMeetingRegistrationFormRepository>().AddAsync(form); // Send the registration to repository to be added to database
    ...
    public async Task<Result> UpdateFormAsync(UserMeetingRegistrationFormDto form, Guid id)
    {
        ...
        try
        {
            ...
            var success = await _uow.CreateRepository<IUserMeetingRegistrationFormRepository>()
    ...
    public async Task<Result> DeleteFormAsync(Guid id)
    {
        try
        {
            var success = await _uow.CreateRepository<IUserMeetingRegistrationFormRepository>().RemoveAsync(id);
    ...
    public async Task<Result<List<UserRegistrationInfoDto>>> GetRegistrationInfoByUserIdAsync(Guid id)
    {
        ...
        try
        {
            var forms = await _uow.CreateRepository<IUserMeetingRegistrationFormRepository>()
            ...
            var registrationInfoDtoList = forms.Select(x => new UserRegistrationInfoDto(
                    ...
                    x.Allergies,
                    x.UserInformation,
                    x.Event.Invitation != null ? x.Event.Invitation.ContactInfo : "Albin Wukics, +46 700 233 533",
                    x.Event.Invitation != null ? x.Event.Invitation.ContactEmail : "albin.wukics@cgi.com",
                    x.Event.Name,
                    x.Event.EventDate))
                .ToList();
    ...
    public async Task<Result> SendWelcomeMessageWithAccountConfirmation(UserMeetingRegistrationForm user, string password)
    {
        ...
        try
        {
            ...
            //Får inte med mig appsettings hä`??

            await _emailService.SendEmailAsync(user.Email, "Välkommen från oss på Fagus", message.Value); // TODO: Denna aktiveras i ett senare skede
    ...
    {
        //TODO Gå igenom detta när emailservice är på plats 
        try
        {
            ...
            //var uriBuilder = new UriBuilder("https://localhost:44310/")
            var uriBuilder = new UriBuilder("https://fagususermeeting.com/")
            {
                //Scheme = "https",
            ...
            {
                return Result.Ok($"<h2>Välkommen till Fagus Användarmöte!</h2> " +
                                 ...
                                 $"<p>Bästa hälsningar, <br>Fagus-teamet</p>");
            }
